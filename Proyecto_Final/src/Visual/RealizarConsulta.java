...
public class RealizarConsulta extends JDialog {
    ...
    public RealizarConsulta(Cita cita) {
        ...
        if (cita.getCliente().getHistorial() != null) {
            if (cita.getCliente().getHistorial().getConsultas() != null && !cita.getCliente().getHistorial().getConsultas().isEmpty()) {
                tieneHistorial = true;
            }
        }
        ...
        btnHistorial.setBounds(20, 640, 200, 50); 
        ...
        btnTerminar.setBounds(230, 640, 250, 50); 
        contentPanel.add(btnTerminar);

        JButton btnCancelar = new JButton("Cancelar");
        Estilos.estilarBoton(btnCancelar, new Color(231, 76, 60), Color.WHITE);
        btnCancelar.setBounds(490, 640, 130, 50); 
        contentPanel.add(btnCancelar);

        JButton btnVacuna = new JButton("Aplicar Vacuna");
        Estilos.estilarBoton(btnVacuna, new Color(110, 140, 251), Color.WHITE);
        btnVacuna.setBounds(630, 640, 160, 50); 
        contentPanel.add(btnVacuna);
        btnVacuna.setForeground(Color.BLACK);

        // --- ACCIONES DE BOTONES (Con manejo de Hilos para Vacunas) ---
        btnVacuna.addActionListener(e -> {
            new Thread(() -> {
                Object res = ClienteSocket.enviar("LISTAR_VACUNAS", null);
                if (!(res instanceof ArrayList)) {
                    SwingUtilities.invokeLater(() -> JOptionPane.showMessageDialog(null, "No hay vacunas o error de conexión."));
                    return;
                }
                ArrayList<Vacuna> vacunas = (ArrayList<Vacuna>) res;
                if (vacunas.isEmpty()) {
                    SwingUtilities.invokeLater(() -> JOptionPane.showMessageDialog(null, "No hay vacunas en el sistema."));
                    return;
                }
    ...
    // --- BLINDAJE CONTRA NULL POINTER EXCEPTION EN SERVIDOR ---
    private void terminarConsulta() {
        ...
        // 1. BLINDAJE: Si el cliente no tiene historial (es nuevo), lo inicializamos
        if (citaActual.getCliente().getHistorial() == null) {
            citaActual.getCliente().setHistorial(new Historial());
            // 2. ENVIAMOS LA ACTUALIZACIÓN DEL CLIENTE AL SERVIDOR PRIMERO
            ClienteSocket.enviar("UPDATE_CLIENTE", citaActual.getCliente());
        }
        ...
        citaActual.setEstado("Completada");
        
        // 4. Enviar actualización de cita
        ClienteSocket.enviar("EDIT_CITA", citaActual);
        ...
        if (guardado) {
            ...
            JOptionPane.showMessageDialog(null, "Error al guardar la consulta. Verifique la conexión.");
        }
    }
    ...
    private void verHistorialFiltrado() {
        // Blindaje extra: Buscar cliente actualizado
        Cliente clienteActualizado = (Cliente) ClienteSocket.enviar("BUSCAR_CLIENTE_CEDULA", citaActual.getCliente().getCedula());
        if (clienteActualizado != null) {
            citaActual.setCliente(clienteActualizado);
        }
        ...
        ArrayList<Consulta> consultasVisibles = citaActual.getCliente().getHistorial()
